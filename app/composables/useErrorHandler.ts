  ...
  const setupGlobalErrorHandler = () => {
    if (typeof window !== 'undefined') {
      ...
      onUnhandledRejectionHandler = (event: PromiseRejectionEvent) => {
        handleNetworkError(
          'Promise rejection',
          (event as any).reason?.message || 'Unknown promise rejection'
        )
      }
      window.addEventListener('error', onErrorHandler)
      window.addEventListener('unhandledrejection', onUnhandledRejectionHandler as any)
    }
  }
  ...
  const cleanup = () => {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (onUnhandledRejectionHandler) {
        window.removeEventListener('unhandledrejection', onUnhandledRejectionHandler as any)
        onUnhandledRejectionHandler = null
      }
    }
  }
