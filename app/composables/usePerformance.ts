...
export const usePerformance = (): PerformanceManager => {
  ...
  const perfConfig = computed(() => (runtimeConfig.public.appConfig as any)?.performance || {})
  ...
  const optimizeImages = (): void => {
    ...
    if ('IntersectionObserver' in window) {
      const imageObserver = new IntersectionObserver((entries) => {
        ...
        rootMargin: (perfConfig.value?.optimization?.lazyLoading?.threshold as string) || '100px',
        threshold: 0.01
      })
      ...
      try {
        const longTaskObserver = new PerformanceObserver((list) => {
          list.getEntries().forEach(entry => {
            const threshold = (perfConfig.value?.monitoring?.longTasks?.threshold as number) ?? 50
