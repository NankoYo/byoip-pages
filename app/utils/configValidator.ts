...
export class ConfigValidator {
  ...
  private static validateHostnameOrWildcard(host: string): boolean {
    const wildcardHostRegex = /^(\*\.)?[a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)+$/
    return wildcardHostRegex.test(host)
  }
    ...
    data.services.forEach((service: any, index: number) => {
      ...
      if (Array.isArray(service.optimizedIPs)) {
        service.optimizedIPs.forEach((ip: string, ipIndex: number) => {
          const isValidIp = this.validateIPAddress(ip)
          const isValidHost = this.validateHostnameOrWildcard(ip)
          if (!isValidIp && !isValidHost) {
            errors.push(`服务 ${index}: 地址无效，位于索引 ${ipIndex}: "${ip}"`)
          }
        })
      }
